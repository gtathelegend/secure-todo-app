...
export default function DashboardClient({ initialTodos, initialUser }: DashboardClientProps) {
  ...
  useEffect(() => {
    ...
    setUser(initialUser, null); // JWT is already in cookies, axios interceptor handles it
  }, [initialTodos, initialUser, setTodos, setUser]);
  ...
  const handleAddTodo = async () => {
    ...
    try {
      ...
      const normalized: Todo = {
        id: created.id,
        title: created.title || created.attributes?.title || 'Untitled',
        isCompleted: Boolean(created.isCompleted || created.attributes?.isCompleted),
      };
      
      addTodo(normalized);
      setNewTodo('');
    } catch (error: any) {
      toast.error(error.response?.data?.error?.message || 'Failed to add todo');
    } finally {
      setAdding(false);
    }
  };
  ...
  const toggleTodo = async (id: number, currentStatus: boolean) => {
    ...
    updateTodo({ id, title: previous?.title || '', isCompleted: nextStatus });
    ...
    try {
      ...
      if (previous) updateTodo(previous);
